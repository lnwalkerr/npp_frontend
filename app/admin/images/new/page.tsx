...
export default function CreateImagesPage(): JSX.Element {
  ...
  const handleSetCoverPhoto = (photoId: number, imageIndex: number): void => {
    setFormData((prev) => ({
      ...prev,
      photos: prev.photos.map((photo) =>
        photo.id === photoId
          ? { ...photo, isCover: true, coverPhotoIndex: imageIndex }
          : { ...photo, isCover: false, coverPhotoIndex: null },
      ),
    }));
  };
        ...
        setFormData((prev) => ({
          ...
          photos: prev.photos.map((photo) => {
            if (photo.id === photoId) {
              ...
              // Auto-set cover photo logic
              let isCover = photo.isCover;
              let coverPhotoIndex = photo.coverPhotoIndex;

              // If this is the first image being added, make it cover
              if (photo.files.length === 0 && acceptedFiles.length === 1) {
                isCover = true;
                coverPhotoIndex = 0;
              }
